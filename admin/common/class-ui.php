...
class ui  {
    ...
    function buildTable($data,$order=0,$filter=0, $filtericon=""){
        $row = 0;
        $tr  = "";
        ...
        if ($data != ""){
            foreach((array)$data as $k=>$v){    
                ...
                foreach($v as $sk=>$sv) { 
                    ...
                    if ($_GET["fireUI"]["filter"][$sk] != "") $img = "<span class='fireUI_filter'>&#8226;</span>"; else $img = $filtericon;
                    if($row==1){
                        $th .= "\n  <th><span value=\"$sk\">". $labelsk."</span>$img" ; 
                                 if ($filter) $th .= "\n  <div class='fireUI_boxfilter'><input type=\"text\" placeholder='filter' name=\"fireUI[filter][$sk]\" value=\"".htmlspecialchars($_GET["fireUI"]["filter"][$sk])."\"/></div>"; 
                        $th .= "</th>";
                        }      
                                 $tr .= "\n  <td><span>$sv</span></td>" ; }
                $tr .= "\n</tr>";
            ...
            if ($_GET["fireUI"]["filter"]!= ""){
                $out .= "No matches found. <a href='".$_SERVER['HTTP_REFERER']."'>Undo filter</a>";
                return $out;
            } 
        }
